...
- **[2026-04-21 security]** **Audit code quality — critiques appliquées** : (1) Secret hardcodé `OXAPAY_API_KEY` retiré de `tests/test_oxapay_ab.py:279` → lu depuis `os.environ["OXAPAY_MERCHANT_API_KEY"]`, skip si absent. (2) Hardcoded admin password retiré de `tests/test_admin_manual_order.py:14` → lu depuis `os.environ.get("ADMIN_PASSWORD", ...)` avec fallback identique à `server.py:3814`. (3) `REACT_APP_BACKEND_URL` du test rendu safe (`.get(..., "")` au lieu de `.get().rstrip()` qui crashait si absent). Faux positifs ignorés après vérification : `server.py:1210` (`unit` assigné dans `else:` final), `server.py:2861/2911` (vars assignées dans `try:` dont `except` re-raise), `i18n.js:190/348` (labels "Password" des traductions admin login, pas des secrets).
