...
- **[2026-04-21 refactor]** **Option C — Session 1 : split OrderConfirmation.js** (664 → 392 lignes, -41%). Extraction de 6 sous-composants dans `/app/frontend/src/components/confirmation/` : `Confetti.js` (40 particles), `StatusBadge.js` (4 variants completed/pending/payment_mock/failed), `LinkCard.js` (single link row), `LinksSection.js` (list + action bar copy-all/download/share), `OrderSummaryCard.js` (order info card), `StripeBanners.js` (StripeCancelBanner + StripePollingBanner exports). Testing agent iteration_14 : **17/17 checkpoints PASS** (desktop + mobile), aucune régression fonctionnelle. Ajouts post-test : `data-testid` ajoutés sur `order-summary-card`, `links-section`, `link-card-{idx}`, `copy-link-btn-{idx}`, `open-link-btn-{idx}`, `copy-all-btn`, `download-txt-btn`, `share-btn`, `status-badge-{status}`, `mock-confirm-btn`. i18n du LinkCard : titres "Ouvrir"/"Copier" passés en 8 langues via `pickLang`.
- **[2026-04-21 config]** **Production keys injectées dans `/app/backend/.env`** (gitignored) : `SMTP_SERVER/PORT/USERNAME/PASSWORD/FROM_EMAIL/FROM_NAME` (mail.deezlink.com SSL 465, inscription@deezlink.com), `STRIPE_API_KEY` (sk_live_...), `OXAPAY_MERCHANT_API_KEY`, `ADMIN_PASSWORD`. Backend restart OK, `/api/packs` 200, `/api/security/token/init` 200. ⚠️ **IMPORTANT** : l'utilisateur a exposé ces clés dans le chat — il doit les rotate en prod.
- **[2026-04-21 cleanup]** **Option B audit appliquée — hooks deps + array keys** : (1) `ClickCaptchaWidget.js:88` — seul vrai warning `react-hooks/exhaustive-deps` du build : `verifyChallenge` wrappé en `useCallback([onVerified])`, puis ajouté aux deps de `startChallenge`. (2) Array keys dynamiques remplacées par des IDs stables : `AdminDashboard.js` (realLinks→`link.id||url`, users→`u.id||email`, blocked_ips→`item.ip`, securityLogs→composite `event-ip-created_at`), `Checkout.js:394` multi_lines→`l.pack_id`, `OrderConfirmation.js:513` LinkCard→`link.id||url||idx`, `Offers.js:441` VOLUME_TIERS→`${min}-${max}`. (3) Les autres `key={i}` (Checkout.js steps/trust bullets, OrderConfirmation confetti particles, static FAQ/features arrays) gardés en index car arrays **statiques**. Build React : **Compiled successfully** (0 warning).
- **[2026-04-21 security]** **Option A audit appliquée — secrets hardcodés** : (1) `OXAPAY_API_KEY` retiré de `tests/test_oxapay_ab.py:279` → lu depuis `os.environ["OXAPAY_MERCHANT_API_KEY"]`, skip si absent. (2) Admin password retiré de `tests/test_admin_manual_order.py:14` → lu depuis `os.environ.get("ADMIN_PASSWORD", ...)` avec fallback identique à `server.py:3814`. (3) `REACT_APP_BACKEND_URL` du test rendu safe. Faux positifs : `server.py:1210` (`unit` assigné dans `else:` final), `server.py:2861/2911` (vars dans `try:` dont `except` re-raise), `i18n.js:190/348` (labels "Password" des traductions).
- **[2026-04-21 bug]** **Header mobile fix overscroll** : ajout de `overscroll-behavior-y: none` sur `html` + `body` dans `index.css` → kill le rubber-band d'iOS Safari qui pouvait dégager visuellement le header `position: fixed` quand l'utilisateur tirait la page vers le haut au niveau du Hero.
- **[2026-04-21 bug]** **Animations cards + reveals mobile restaurées** : (1) La card active du carrousel mobile se sur-élève maintenant de 6px via framer-motion (`y: isActive ? -6 : 0` sur le wrapper MobileCarousel) — le CSS `transform: translateY` ne fonctionnait pas parce que `.feature-card` est la motion.div de `StaggerItem`, et framer-motion clobbait la CSS transform avec son inline style. (2) Halo violet de la card active renforcé pour Features, Pricing (landing) + Offers pack grid (via nouveau attribut `[data-pack-card="plain|highlight"]` appliqué sur `pages/Offers.js`). (3) **Reveal/StaggerItem** : supprimé le downgrade "fade only" sur mobile — les spring physics (stiffness 90/95, translateY + scale) s'appliquent maintenant partout. (4) `StaggerItem` fonctionne maintenant en **standalone** (défaut : `initial+whileInView` inline) OU dans un `StaggerGroup` (`standalone={false}`, variants). FeaturesSection et PricingSection passent un `delay={i * 0.08}` pour simuler un stagger naturel, FAQSection garde `standalone={false}` dans son StaggerGroup.
- **[2026-04-21]** **Scroll mobile simplifié — retour au scroll natif** : `frontend/src/utils/smoothScroll.js` n'intercepte plus les touch events sur mobile (détection `matchMedia("(hover: none) and (pointer: coarse)")`). Le scroll natif iOS/Android fournit le momentum/bounce, et le CSS `scroll-snap-type: y proximity` sur `<html>` + `scroll-snap-align: start` sur `section[id]` gère le snap sur les sections. Le moteur JS custom (wheel inertia + LERP 0.10) reste actif uniquement sur desktop. Supprimé : `onTouchStart/Move/End`, `trySnapToSection`, `findNearestSection`, `touchVelocity`, direction lock — tout ça combattait Safari iOS et ratait ses cibles (354px off). Animations sur cards non-actives du carousel figées via CSS `[data-carousel-slide]:not([data-active="true"])` dans `index.css`.
- **[2026-04-20 bug]** **Audit mobile intégral + fixes critiques** : (1) Dots carrousel verticaux (40px de haut au lieu de 6px) — causés par la règle CSS globale `min-height:40px` sur `button, [role="tab"]` dans index.css → exclusion des `[data-testid^="carousel-dot-"]`. (2) Burger menu mobile non fonctionnel — le wrapper `fixed z-50 pointer-events-none` du Header propage à ses enfants ; le panel du menu mobile n'avait pas `pointer-events-auto` → ajouté + `data-testid=mobile-menu-panel`. (3) Opacité du panel menu augmentée (0.92 → 0.97) + backdrop-blur 2xl pour lisibilité. Audit testing_agent iteration_13 : 18/18 mobile interaction tests PASS.
- **[2026-04-20]** **Panel Admin — ajout manuel** : nouveau endpoint `POST /api/admin/orders/manual-create` (pack prédéfini ou quantité custom, assignation stock/liste explicite, option email, points fidélité). UI `AdminDashboard.js` : formulaire "Ajouter une commande manuelle" (Commandes) + "Ajouter un lien"/"Importer en masse" (Liens). Testing agent iteration_12 : 12/12 backend pytest PASS.
