...
export default function Login() {
  ...
  // Subscribe to telemetry state changes for conditional captcha
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (requireCaptcha && !captchaToken) { setError(T.captchaRequired); return; }
    ...
    try {
      ...
      // Only include captcha_token if captcha was required and solved
      if (captchaToken) {
        payload.captcha_token = captchaToken;
      }
      
      const result = await securePost("/auth/magic", payload);
      ...
      if (status === 429) {
        ...
        // Check if server says captcha required
        const code = typeof detail === "object" ? detail?.code : "";
        if (code === "CAPTCHA_REQUIRED") {
          setRequireCaptcha(true);
          setCaptchaToken("");
          setError(typeof detail === "object" ? detail.message : T.captchaRequired);
        } else {
          setCaptchaToken("");
          setError(typeof detail === "string" ? detail : typeof detail === "object" ? detail.message : T.genericError);
        }
